...
public class Zol implements PageProcessor{
	...
	public Apk process(Page page) {
		...
		List<String> urls =page.getHtml().links().regex("http://sj\\.zol\\.com\\.cn/.*").all() ;
		
 		
		Set<String> cacheSet = Sets.newHashSet();
		cacheSet.addAll(urls);
		for (String temp : cacheSet) {
			if(!temp.contains("http://sj.zol.com.cn/down.php?softid=") && PageProUrlFilter.isUrlReasonable(temp))				
				page.addTargetRequest(temp);
		}
		
	
		//提取页面信息
		if(page.getUrl().regex("http://sj\\.zol\\.com\\.cn/.+").match()){
