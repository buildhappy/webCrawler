...
public class PagePro155 implements PageProcessor {
    ...
    public Apk process(Page page) {
        ...
        // 获取搜索页面
        if (page.getUrl().regex("http://android\\.155\\.cn/|game|soft").match()) {
            LOGGER.debug("match success, url:{}", page.getUrl());

            // 获取详细链接，以及分页链接
            List<String> urlList = page.getHtml().links().regex("http://android\\.155\\.cn/game/|soft/").all();
            urlList.addAll(page.getHtml().links().regex("http://android\\.155\\.cn/.*").all());
