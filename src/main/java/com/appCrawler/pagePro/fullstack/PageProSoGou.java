...
public class PageProSoGou implements PageProcessor {
    ...
    public Apk process(Page page) {
        ...
        // 获取搜索页面
        if (page.getUrl().regex("http://app\\.sogou\\.com/|game|soft").match()) {
            LOGGER.debug("match success, url:{}", page.getUrl());

            // 获取详细链接，以及分页链接
            List<String> urlList = page.getHtml().links().regex("http://app\\.sogou\\.com/game|soft").all();
            urlList.addAll(page.getHtml().links().regex("http://app\\.sogou\\.com/detail/.*").all());
