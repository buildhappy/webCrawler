...
public class PageProPconline implements PageProcessor {
    ...
    public Apk process(Page page) {
        ...
        // 获取搜索页面
        if (page.getUrl().regex("http://ks.pconline.com.cn/download.shtml\\?q=*").match()) {
            LOGGER.debug("match success, url:{}", page.getUrl());

            // 获取详细链接，以及分页链接
            List<String> urlList = page.getHtml().links("//div[@class='dlList']/ul/li").all();
            urlList.addAll(page.getHtml().links("//div[@id='JPage']").all());
