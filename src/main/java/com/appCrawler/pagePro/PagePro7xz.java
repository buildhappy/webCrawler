...
public class PagePro7xz implements PageProcessor {
    ...
    public Apk process(Page page) {
        ...
        // 获取搜索页面
        if (page.getUrl().regex("http://www\\.7xz\\.com/ng/search.*").match()) {
            LOGGER.debug("match success, url:{}", page.getUrl());

            // 获取详细链接，以及分页链接
            List<String> urlList = page.getHtml().links("//div[@class='classes-list-con']/div/h2").all();
            urlList.addAll(page.getHtml().links("//div[@class='Pagebox']").all());
