...
public class PagePro155 implements PageProcessor {
    ...
    public Apk process(Page page) {
        ...
        // 获取搜索页面
        if (page.getUrl().regex("http://android.155.cn/search.php\\?").match() && !page.getUrl().get().contains("index=news") && !page.getUrl().get().contains("index=tags")) {
            LOGGER.debug("match success, url:{}", page.getUrl());

            // 获取详细链接，以及分页链接
            List<String> urlList = page.getHtml().links("//ul[@class='gmc-c']/li").all();
