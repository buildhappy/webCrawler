...
public class PagePro97sky implements PageProcessor {
    ...
    public Apk process(Page page) {
        ...
        // 获取搜索页面
        if (page.getUrl().regex("http://www\\.97sky\\.cn/search\\?type=0&keyword=.*").match()) {
            LOGGER.debug("match success, url:{}", page.getUrl());

            // 获取详细链接，以及分页链接
            List<String> urlList = page.getHtml().links("//li[@class='ppInCont']/div[@class='ppLimg']").all();
            urlList.addAll(page.getHtml().links("//div[@class='ppPage']").all());
